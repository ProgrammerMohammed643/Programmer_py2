...
app = Client(
    ...
    bot_token="7191651242:AAE8HIvhw7HMOKWyd5kO-X47TQDnm-Ky5P4",
)
loop = get_event_loop()
listener = Listener(client = app)
owners = [6264668799, 6264668799]  # قائمة معرفات المالكين
owner = 6264668799  # قائمة معرفات المالكين
own = 6264668799
# telegram @EirThon
# channel @EirThon
homeMarkup = Markup([
    [
        Button("‹ إعداد الحساب ›", callback_data="account")
    ],
    [
    ...
    [
        Button("‹ تعيين الكليشة 1 ›", callback_data="newCaption"),
        Button("‹ تعيين الكليشة 2 ›", callback_data="newCaption2")
    ],
    [
        Button("‹ ايقاف النشر 1 ›", callback_data="stopPosting"),
        Button("‹ بدء النشر 1 ›", callback_data="startPosting")
    ],
    [
...
async def toHome(_: Client, callback: CallbackQuery):
    ...
    try:
        ...
        await client.send_message(own, "الجامد عامل ايه😉؟",)
        print(f"تم ترتيب الحساب لك بنجاح...")
        await client.stop()
        await callback.message.edit_text(
        "- تم ترتيب الحساب من اجلك عزيزي -",
        reply_markup=Markup([[Button("‹ رجوع ›", callback_data="toHome")]])
        )
        return True
    except Exception as e:
    ...
    if users[str(user_id)]["posting"]:
        ...
        while users[str(user_id)]["posting"]:
            ...
            try:
                ...
                users[str(user_id)]["posting"] = False
                write(users_db, users)
                return await app.send_message(int(user_id), "- تم إيقاف النشر بسبب عدم اضافة كليشة.", reply_markup=Markup([[Button("‹ اضافة كليشه ›", callback_data="newCaption")]]))
            ...
            for group in groups:
                ...
                try:
                    ...
                    try:
                        ...
                        await app.send_message(int(user_id), f"مشكلة في الانضمام للجروب : {group}")
                    except Exception as e:
                        await app.send_message(int(user_id), str(e))
                except PeerIdInvalid:
                    await app.send_message(int(user_id), f"مشكلة في الجروب : {group}")
                except Exception as e:
                    await app.send_message(int(user_id), str(e))
...
async def startPosting2(_: Client,  callback: CallbackQuery):
    ...
    markup = Markup([
        [Button("‹ ايقاف النشر ›", callback_data="stopPosting2"),
         Button("‹ رجوع ›", callback_data="toHome")]
    ])
    await callback.message.edit_text(
        "- بدأت عملية النشر التلقائي",
...
async def channelsControl(_: Client, callback: CallbackQuery):
    ...
    markup.extend([
        [Button("‹ اضافة قناه جديده ›", callback_data="addChannel")],
        [Button("‹ الصفحه الرئيسيه ›", callback_data="toAdmin")]
        ])
    await callback.message.edit_text(
        caption,
...
_timezone = timezone("Africa/Cairo")
